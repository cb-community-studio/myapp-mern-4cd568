...
import sheet12FakerFactory from "./sheet12FakerFactory";
...
const Sheet12FakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = sheet12FakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("sheet12").create(elem));
...
export default connect(null, mapDispatch)(Sheet12FakerDialogComponent);
